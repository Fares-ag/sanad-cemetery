...
export function AwqafLoginPage() {
  const { login, currentUser } = useAwqafAuth();
  const { t } = useI18n();
  const navigate = useNavigate();
  const loc = useLocation();
  const from = (loc.state as { from?: string } | null)?.from ?? '/';

  const [email, setEmail] = useState('admin@awqaf.sanad.local');
  ...
  return (
    <div className="dash-shell awqaf-dash" style={{ minHeight: '100vh' }}>
        ...
        <div className="dash-page" style={{ maxWidth: 440, margin: '0 auto', paddingTop: '4rem' }}>
          ...
          <div className="panel">
            ...
            <form onSubmit={onSubmit} className="stack" style={{ gap: '1rem' }}>
              ...
              <div className="field">
                <label htmlFor="awq-email">{t('login.email')}</label>
                <input
                  id="awq-email"
                  type="email"
              ...
              <div className="field">
                <label htmlFor="awq-pw">{t('login.password')}</label>
                <input
                  id="awq-pw"
                  type="password"
